...
def parseArgs():
    usage = """
    Reposyncs3 is used to synchronize a remote yum repository to a s3 bucket
    using yum to retrieve the packages.
...
def main():
    (opts, junk) = parseArgs()
    # TODO: Check if bucket exists and you have access
    ...
    for repo in my.repos.listEnabled():
        ...
        for pkg in download_list:
            ...
            try:
                ...
                # TODO: Push to s3 bucket from local
                k = Key(s3bucket)
                k.key = repo.id + '/' + remote
                my.logger.info( 'Copy Package from path[%s] to s3[%s]' % (path, k.key))
